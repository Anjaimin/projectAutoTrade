...
class MyWindow(QMainWindow, form_class):
    ...
    def trade_stocks(self):
        ...
        try:
            today = datetime.datetime.now().strftime('%Y%m%d')
            #self.textboard.append(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Checking for nearest business day using date: {today}")

            # 디버깅 코드 시작
            from pykrx.website.krx import get_nearest_business_day_in_a_week as krx_get_nearest_business_day_in_a_week

            # KRX 내부 함수 호출 및 결과 확인
            try:
                nearest_business_day = krx_get_nearest_business_day_in_a_week(today, prev=True)
                #self.textboard.append(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Nearest business day for {today}: {nearest_business_day}")
            except Exception as e:
                self.textboard.append(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Error2: {e}")
                return
            ...
            self.textboard.append(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Nearest business day found: {yesterday}")
                    ...
                    try:
                        ...
                        if not yesterday_data.empty:
                            ...
                            if current_price > target_price:
                                if self.buy_stock(code.strip(), current_price, 10):
                                    self.bought_list[code.strip()] = today
                    except Exception as e:
                        self.textboard.append(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Error3: {e}")
                        continue

        except Exception as e:
            self.textboard.append(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Error1: {e}")
    ...
    def buy_stock(self, code, price, quantity):
        ...
        self.textboard.append(f"[DEBUG] Checking for account_number: {account_number}")  # 디버깅을 위해 계좌번호 출력
        order_type = 1  # 매수
        order_result = self.kiwoom.SendOrder("매수주문", "0101", account_number, order_type, code, quantity, price,
                                             "03", "")
        ...
        # account_number = self.kiwoom.GetLoginInfo("ACCNO")[1]
        # self.textboard.append(f"[DEBUG] Checking for account number: {account_number}")
        #
        # order_type = 1
    ...
    def sell_all_stocks(self):
        ...
        self.textboard.append(f"[DEBUG2] Checking for account number: {account_number}")
        ...
        if '종목번호' in stocks_info:
            for idx, code in enumerate(stocks_info['종목번호']):
                ...
                if quantity > 0:
                    order_type = 2
                    order_result = self.kiwoom.SendOrder("매도주문", "0101", account_number, order_type, code, quantity, 0,
                                                         "03", "")
                    if order_result == 0:
